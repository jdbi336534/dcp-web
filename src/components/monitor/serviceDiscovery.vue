            ...
            <el-table :data="servicediscoverylist" :resizable="true" border style="width: 100%;" :loading="false" element-loading-text="拼命加载中">
                <el-table-column label="名称" width="200" show-overflow-tooltip>
                    <template scope="scope">
                        ...
                        <span style="margin-left: 10px"><router-link :to="{name:'servicedetails',params:{ip:{ip:scope.row.metadata.name,father:'/dcp/monitor/serviceDiscovery',nametitle:'服务发现'}}}">{{scope.row.metadata.name}}</router-link></span>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="内部端口" prop="inner" show-overflow-tooltip>
                ...
                <el-table-column label="外部端口" prop="outer" show-overflow-tooltip>
                    <template scope="scope">
                        {{`${scope.row.metadata.name}:${scope.row.spec.ports[0].port}${scope.row.spec.ports[0].protocol}`}}
                    </template>
                </el-table-column>
