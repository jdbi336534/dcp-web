...
// 获得虚拟节点列表
var getPods = (virnode) => fetch('GET', `${COMMON}/api/v1/namespaces/${PERMISSIONS}/pods/${virnode}`);
// 删除虚拟节点
var deletePods = (virnode) => fetch('DELETE', `${COMMON}/api/v1/namespaces/${PERMISSIONS}/pods/${virnode}`);
// 从此接口中获取下拉选框的第一级
var getfirstCascader = () => fetch('GET', `${COMMON}/api/v1/namespaces/${PERMISSIONS}/services`);
// 从此接口中获取在线日志,返回的是字符串，所以不能用这个
var getLogs = (pod, succed, failed) => $.ajax({ url: `${COMMON}/api/v1/namespaces/${PERMISSIONS}/pods/${pod}/log`, 'success': succed, 'error': failed });
// 离线日志中的下拉选框中的数据
...
// 新增服务接口
var newService = (name) => fetch('POST', `${COMMON}/apis/extensions/v1beta1/namespaces/${PERMISSIONS}/deployments/${name}/`);
// 删除服务接口
var delService = (name) => fetch('DELETE', `${COMMON}/apis/extensions/v1beta1/namespaces/${PERMISSIONS}/deployments/${name}/`);
// 查询一个或者全部的服务
var queryService = (name) => fetch('GET', `${COMMON}/apis/extensions/v1beta1/namespaces/${PERMISSIONS}/deployments/${name}/`);
/** *****************************************蒋东兵***************************************************/
/** *****************************************赵凯芳***************************************************/
// 服务管理接口
...
// node 集群cpu
var nodeclusterchart = (obj) => fetch('GET', `${COMMON}${CHARTONE}/api/v1/model/metrics/${obj.type}/${obj.name}`);
